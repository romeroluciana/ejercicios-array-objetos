...
const artistasSolistas = (artistas) =>
  artistas.filter(({ solista }) => solista);
...
const artistasPorEdad = (edadElegida, artistas)=> artistas.filter(({ edad }) => edad==edadElegida);

console.log(artistasPorEdad(34,artistas));
...
const cantidadDeArtistasPorInstrumento = (artistas)=> {
    ...
    for(const artista of artistas){
        if(acc[artista.instrumento]==undefined){
...
const cantidadDeArtistasPorGenero = (artistas) => {
  ...
  for (const artista of artistas) {
    ...
    // generos["punk"]
    // generos.punk
    if (generos[artista.genero] == undefined) {
      //generos.punk = 1
      generos[artista.genero] = 1;
    } else {
      generos[artista.genero]++;
    }
  }
...
const cantidadDeArtistasPorGenero1 = (artistas) => {
  const generos = {};
...
const cantidadDeArtistasPorGenero2 = (artistas) =>
  artistas.reduce((generos, artista) => {
    if (generos[artista.genero] == undefined) {
      generos[artista.genero] = 1;
    } else {
      generos[artista.genero]++;
...
//   const artistasConMasDisco = artistas.filter(({discos})=>(discos.length >= cantidadDeDiscos))
//   // const artistasConMasDiscoOrdenado = artistasConMasDisco.sort(({discos:discosA},{discos:discosB})=>{
//   //   return discosB.length - discosA.length
//   // })

//   const artistasConMasDiscoOrdenado = artistasConMasDisco.sort((a,b)=>{
//       return b.discos.length - a.discos.length
//     })

//   return artistasConMasDiscoOrdenado
// }

const artistasConMasDiscosQue = (cantidadDeDiscos, artistas) =>
  artistas.filter(({ discos }) => discos.length >= cantidadDeDiscos).sort((a, b) => b.discos.length - a.discos.length);

console.log(artistasConMasDiscosQue(3, artistas));
...
const artistasConMasEntradasVendidas = (artistas)=>{
  const artistasOrdenados = artistas.sort((artistasA,artistasB)=>{
    return artistasB.ultimoRecital.entradasVendidas- artistasA.ultimoRecital.entradasVendidas
  })
  return artistasOrdenados[0]
}
console.log(artistasConMasEntradasVendidas(artistas))
console.log(artistas[0].ultimoRecital.entradasVendidas);

const artistaConMasEntradasVendidas2 = (artistas) =>{
  let aux = 0
  let artistaEntra = {}
  artistas.forEach(artista  => {
   console.log(artista.ultimoRecital.entradasVendidas);
    if( artista.ultimoRecital.entradasVendidas > aux){
      aux = artista.ultimoRecital.entradasVendidas
      artistaEntra = artista
    }
});
console.log(aux);
return artistaEntra
...
console.log(artistaConMasEntradasVendidas2(artistas));


const artistaConMasEntradasVendidas3 = (artistas) =>{
  let aux = artistas.shift()
 
  artistas.forEach(artista  => {
    if( artista.ultimoRecital.entradasVendidas > aux.ultimoRecital.entradasVendidas){
      aux = artista
...
console.log(artistaConMasEntradasVendidas3(artistas));
...
const artistaConMayorRecaudacion =(artistas)=>{
  ...
  let recaudacion = {}
   ...
   artistas.forEach(
     artista =>{
       const valorEntrada = artista.ultimoRecital.entradasVendidas * artista.ultimoRecital.costoEntradas
       if( valorEntrada > aux){
         aux = valorEntrada
         recaudacion = artista
       } 
...
const artistasConDiscoEnAnio1 = (anio, artistas)=>{

  return artistas.some(artista =>{
    console.log(artista);
    return artista.discos.some(disco => {
      console.log(disco);
...
console.log(artistasConDiscoEnAnio1(2003, artistas));
